  ...
  const handleGoBack=()=>{
   navigate(-1)
    ...
    return (
       ...
       <button onClick={()=>handleGoBack()}  className="mb-4 text-teal-600 hover:underline flex items-center"><ChevronLeft size={16}/>Go Back </button>
       
       <div className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col md:flex-row">
          <div className="md:w-1/3 h-64 md:h-auto relative"> 
             <img src={habit.photoURL} alt={habit.title} className="w-full h-full object-cover" />
             <div className="absolute top-4 left-4 bg-black/60 text-white px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1">
                <Flame size={14} className="text-orange-500" /> {habit.effectiveStreak} Day Streak
             </div>
          </div>
          ...
          <div className="p-8 md:w-2/3 flex flex-col">
             <div className="flex justify-between items-start">
                <div>
                   ...
                   <span className="inline-block bg-teal-100 text-teal-800 text-xs px-2 py-1 rounded-full mt-2 font-semibold">{habit.category}</span>
                </div>
             </div>
             
             <p className="mt-6 text-gray-600 text-lg leading-relaxed">{habit.description}</p>
             ...
             <div className="mt-8">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2.5">
                   <div className="bg-teal-600 h-2.5 rounded-full transition-all duration-1000" style={{ width: `${habit.progress}%` }}></div>
                </div>
             </div>
             ...
             <div className="mt-auto pt-8 flex items-center justify-between">
                <div className="flex items-center gap-3">
                   ...
                   <div className="text-sm">
                      <p className="text-gray-900 font-medium">{habit.creatorName}</p>
                ...
                >
                  {habit.status ? <><Check size={20}/> Completed Today</> : 'Mark Complete'}
                </button>
             </div>
          </div>
       </div>
